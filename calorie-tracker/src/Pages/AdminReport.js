...
const AdminReport = () => {
    ...
    const previousWeekEntries = foods.filter(food => new Date(food.dateTime) < last7Days && new Date(food.dateTime) >= previous7Days);

    const currentWeekCount = currentWeekEntries.length;
    const previousWeekCount = previousWeekEntries.length;

    const currentWeekCalories = currentWeekEntries.reduce((total, food) => total + food.calories, 0);
    const avgCaloriesPerUser = currentWeekCalories / currentWeekEntries.length;

    const data = [
        { name: 'Last 7 Days', Entries: currentWeekCount, Calories: currentWeekCalories },
        { name: 'Previous 7 Days', Entries: previousWeekCount, Calories: previousWeekEntries.reduce((total, food) => total + food.calories, 0) }
    ];
